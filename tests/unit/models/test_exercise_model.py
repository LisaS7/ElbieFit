...
def test_exercise_id_comes_from_suffix_of_SK(make_exercise):
    exercise = make_exercise(SK="EXERCISE#1234-uuid")
    assert exercise.exercise_id == "1234-uuid"


def test_exercise_type_must_be_literal_exercise(make_exercise):
    # type is a Literal["exercise"], so anything else should blow up
    with pytest.raises(ValidationError):
        make_exercise(type="banana")


def test_exercise_category_is_optional_and_defaults_to_none(make_exercise):
    exercise = make_exercise(category=None)
    assert exercise.category is None


def test_to_ddb_item_converts_datetimes_using_dt_to_iso(monkeypatch, make_exercise):
